...
contract MasterChefWhitelisted is Ownable {
    ...
    // The block number when JUTC mining starts (1 years).
    uint256 public startBlock;
    // The block number when JUTC mining ends. (5 years + 100 days)
    ...
    modifier onlyWhitelist() {
        require(isWhitelisted[_msgSender()], "Whitelist: caller is not in the whitelist");
        _;
    }
    ...
    function unsetWhitelist(address _address) public onlyOwner {
      isWhitelisted[_address] = false;
    }

    function checkIsWhitelist(address _address) public view returns (bool) {
    ...
    // Lock JUTC tokens to MasterChef for JUTC farm V2 allocation.
    function deposit(uint256 _pid, uint256 _amount) public onlyWhitelist {
    		require(block.number > 15240000, "14-21 Feb 2022 only");
    		require(block.number < 15441600, "14-21 Feb 2022 only");
        PoolInfo storage pool = poolInfo[_pid];
    ...
    // Can't withdraw JUTC up until 5 years
    function withdraw(uint256 _pid, uint256 _amount) public onlyWhitelist {
    	  require(block.number > endBlock, "Lock JUTC for 5 years");
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
